...
- **Формат ошибок**: при любой ошибке выполнения (osascript non-zero exit, нет разрешения, неизвестная команда, невалидные args) сервер отвечает `{ ok: false, error }`. HTTP-код для REST: `500` для ошибки команды, `401` для невалидного токена, `400` для нечитабельного JSON. WS соответствующих кодов не имеет — всё через тело `WireResponse`.
